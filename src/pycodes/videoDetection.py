...
def saveImage(path, videoName, des):
    ...
    while True:
        ret, frame = cap.read()
        if ret == False: break
        if (num % 20 == 0) and (count <= 5):
            cv2.imwrite(os.path.join(des + "/0/", videoName[:-4] + "_" + str(num) + ".png"), frame)
            count += 1
        num += 1
    cap.release()


def videoToImage(videoPath, savePath):
    videoName = videoPath.split('/upload/')[1]
    videoNum = videoName.split('.')[0]
    if os.path.exists(os.path.join(savePath, videoNum)):
        shutil.rmtree(os.path.join(savePath, videoNum))
    os.makedirs(os.path.join(savePath, videoNum + "/0/"))
    saveImage(videoPath, videoName, os.path.join(savePath, videoNum))
    return savePath + "/" + videoNum + "/"
...
def server1_ResNet50(event1, event2, event3, event4, image_1, a1, b1, c1,
                     ...
                     p1, event5, event6, c1_mul, c1_res, t1, detectionId):
    event2.clear()
    event4.clear()
...
def server2_ResNet50(event1, event2, event3, event4, image_, a1, b1, c1,
                     ...
                     conv4_2_3, bn4_2_3,
                     conv4_3_1, bn4_3_1, conv4_3_2, bn4_3_2, conv4_3_3, bn4_3_3, fc,
                     p2, event5, event6, c1_mul, c1_res, t1, detectionId):
    event1.clear()
    event3.clear()
...
def SecResNet50(image_1, image_2, conv0_1, bn0_1, conv1_1_1, bn1_1_1, conv1_1_2, bn1_1_2, conv1_1_3, bn1_1_3, conv1_1_4,
    ...
    server2_process = torch.multiprocessing.Process(target=server2_ResNet50,
                                                    args=(
                                                        ...
                                                        conv4_2_3, bn4_2_3,
                                                        conv4_3_1, bn4_3_1, conv4_3_2, bn4_3_2, conv4_3_3, bn4_3_3, fc,
                                                        p2, event5, event6, c2_mul, c2_res, t2, detectionId))
...
if __name__ == '__main__':
    ...
    # detectionId = 1
    # img_path = "D:/SDNet-backend/upload/80e3ea15e8dd429bb8f287dbc0f66ea8.mp4"
    # img_path = videoToImage("D:/SDNet-backend/upload/80e3ea15e8dd429bb8f287dbc0f66ea8.mp4", "D:/SDNet-backend/upload")

    testData = dsets.ImageFolder(image_path, tran)
    testLoader = torch.utils.data.DataLoader(dataset=testData, batch_size=batch_size, shuffle=False)
    for image, labels in testLoader:
        # image = image.cuda()
        image_1 = torch.tensor(np.random.random(image.shape))
        ...
        result1, result2 = SecResNet50(image_1, image_2, conv0_1, bn0_1, conv1_1_1, bn1_1_1, conv1_1_2, bn1_1_2,
                                       conv1_1_3,
        ...
        res2 = 0
        for item in result1:
            res1 = res1 + item.tolist()[0]
            res2 = res2 + item.tolist()[1]

        print([float('{:.4f}'.format(i)) for i in [res1 / batch_size, res2 / batch_size]])
        res1 = 0
        res2 = 0
        for item in result2:
            res1 = res1 + item.tolist()[0]
        ...
        print([float('{:.4f}'.format(i)) for i in [res1 / 5.0, res2 / 5.0]])
